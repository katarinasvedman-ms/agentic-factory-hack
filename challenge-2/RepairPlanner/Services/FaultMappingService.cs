...
public interface IFaultMappingService
{
    ...
    /// <summary>
    /// Gets the list of part numbers required to repair a specific fault type.
    /// </summary>
    IReadOnlyList<string> GetRequiredParts(string faultType);
}

/// <summary>
/// Provides hardcoded mappings from fault types to required skills and parts.
/// These mappings are specific to tire manufacturing equipment.
/// </summary>
public sealed class FaultMappingService : IFaultMappingService
{
    // Default skills when fault type is unknown
    private static readonly IReadOnlyList<string> DefaultSkills = ["general_maintenance"];
    
    // Default parts when fault type is unknown (empty - no specific parts needed)
    private static readonly IReadOnlyList<string> DefaultParts = [];

    /// <summary>
    /// Maps fault types to the skills required to diagnose and repair them.
    /// </summary>
    ...
    /// <summary>
    /// Maps fault types to the part numbers typically needed for repairs.
    /// Part numbers reference items in the PartsInventory Cosmos DB container.
    /// </summary>
    private static readonly IReadOnlyDictionary<string, IReadOnlyList<string>> FaultToParts =
        new Dictionary<string, IReadOnlyList<string>>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["high_radial_force_variation"] = [],  // No specific parts needed
    ...
    {
        // TryGetValue with case-insensitive comparison
